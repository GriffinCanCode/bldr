...
extern "C" {
    ...
    void launch_vector_scale(const float* input, float* output, float scalar, int n) {
        int blockSize = 256;
        int numBlocks = (n + blockSize - 1) / blockSize;
        vector_scale<<<numBlocks, blockSize>>>(input, output, scalar, n);
    }
}
