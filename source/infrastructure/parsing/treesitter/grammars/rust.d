module infrastructure.parsing.treesitter.grammars.rust;

import infrastructure.parsing.treesitter.bindings;
import infrastructure.parsing.treesitter.config;
import infrastructure.parsing.treesitter.registry;
import infrastructure.parsing.treesitter.parser;
import infrastructure.analysis.ast.parser;
import infrastructure.utils.logging;

/// Rust grammar loader for tree-sitter
/// Dynamically loads grammar from system libraries if available

// Dynamic loader from C
extern(C) const(TSLanguage)* ts_load_rust() @system nothrow @nogc;

private bool grammarLoaded = false;

shared static this() @system {
    try {
        // Try to load grammar dynamically
        auto grammar = ts_load_rust();
        if (!grammar) {
            structuredLog.debug_("rust_grammar_not_available_will_use_file").emit();
            return;
        }
        
        auto config = LanguageConfigs.get("rust");
        if (!config) {
            structuredLog.warning("rust_config_not_found").emit();
            return;
        }
        
        // Register with tree-sitter registry
        TreeSitterRegistry.instance().registerGrammar(
            "rust",
            &ts_load_rust,
            *config
        );
        
        // Create and register parser
        auto parser = new TreeSitterParser(grammar, *config);
        ASTParserRegistry.instance().registerParser(parser);
        
        grammarLoaded = true;
        structuredLog.info("_rust_treesitter_grammar_loaded").emit();
    } catch (Exception e) {
        structuredLog.debug_("rust_grammar_not_loaded_").field("detail", "Rust grammar not loaded: " ~ e.msg).emit();
    }
}

/// Check if Rust grammar is available
bool isRustGrammarAvailable() @safe nothrow {
    return grammarLoaded;
}
